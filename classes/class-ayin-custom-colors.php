<?php
/**
 * Ayin Theme: Custom Colors Class
 *
 * This class is in charge of color customization via the Customizer.
 *
 * Each variable that needs to be updated is defined in the $ayin_custom_color_variables array below.
...
class Ayin_Custom_Colors {

	private $ayin_custom_color_variables = array();
	...
	function __construct() {
		 ...
		 */
		$this->ayin_custom_color_variables = array(
			array( '--global--color-background', '#FFFFFF', __( 'Background Color', 'ayin' ) ),
			array( '--global--color-foreground', '#333333', __( 'Foreground Color', 'ayin' ) ),
			array( '--global--color-primary', '#000000', __( 'Primary Color', 'ayin' ) ),
			array( '--global--color-secondary', '#3C8067', __( 'Secondary Color', 'ayin' ) ),
			array( '--global--color-tertiary', '#FAFBF6', __( 'Tertiary Color', 'ayin' ) ),
			array( '--global--color-border', '#EFEFEF', __( 'Borders Color', 'ayin' ) ),
		);

		/**
		 * Register Customizer actions
		 */
		add_action( 'customize_register', array( $this, 'ayin_customize_custom_colors_register' ) );

		/**
		 * Enqueue color variables for customizer & frontend.
		 */
		add_action( 'wp_enqueue_scripts', array( $this, 'ayin_custom_color_variables' ) );

		/**
		 * Enqueue color variables for editor.
		 */
		add_action( 'enqueue_block_editor_assets', array( $this, 'ayin_editor_custom_color_variables' ) );
	 ...
	 */
	function ayin_color_blend_by_opacity( $foreground, $opacity, $background = null ) {
	 ...
	 */
	function ayin_customize_custom_colors_register( $wp_customize ) {
		...
		$wp_customize->add_control(
			...
			array(
				'type'    => 'radio',
				'section' => 'ayin_options',
				'label'   => __( 'Colors', 'ayin' ),
				'choices' => array(
					'default' => __( 'Theme Default', 'ayin' ),
					'custom'  => __( 'Custom', 'ayin' ),
				),
			)
		);
		 ...
		 */
		foreach ( $this->ayin_custom_color_variables as $variable ) {
			$wp_customize->add_setting(
				"ayin_$variable[0]",
				array(
	 ...
	 */
	function ayin_generate_custom_color_variables( $context = null ) {
		...
		$theme_mod_bg_color = empty( get_theme_mod( 'ayin_--global--color-background' ) ) ? '#FFFFFF' : get_theme_mod( 'ayin_--global--color-background' );

		foreach ( $this->ayin_custom_color_variables as $variable ) {

			if ( ! empty( get_theme_mod( "ayin_$variable[0]" ) ) ) {

				$theme_mod_variable_name = $variable[0];
				$theme_mod_default_color = $variable[1];
				$theme_mod_custom_color  = get_theme_mod( "ayin_$variable[0]" );
				$opacity_integer         = 70;
				$adjusted_color          = $this->ayin_color_blend_by_opacity( $theme_mod_custom_color, $opacity_integer, $theme_mod_bg_color );
		...
		$selection_background = $this->ayin_color_blend_by_opacity( get_theme_mod( 'ayin_--global--color-primary' ), 5, $theme_mod_bg_color ) . ';';
	 ...
	 */
	function ayin_custom_color_variables() {
		if ( 'default' !== get_theme_mod( 'custom_colors_active' ) ) {
			wp_add_inline_style( 'ayin-style', $this->ayin_generate_custom_color_variables() );
		}
	}
	 ...
	 */
	function ayin_editor_custom_color_variables() {
		wp_enqueue_style( 'ayin-custom-color-overrides', get_template_directory_uri() . '/assets/css/custom-color-overrides.css', array(), wp_get_theme()->get( 'Version' ) );
		if ( 'default' !== get_theme_mod( 'custom_colors_active' ) ) {
			wp_add_inline_style( 'ayin-custom-color-overrides', $this->ayin_generate_custom_color_variables( 'editor' ) );
		}
	}
	...
	function on_customize_controls_enqueue_scripts() {
		$handle = 'ayin-wcag-validate-customizer-color-contrast';
		...
		$exports = array(
			'validate_color_contrast' => array(
				...
				'ayin_--global--color-primary'    => array( 'ayin_--global--color-background' ),
				'ayin_--global--color-secondary'  => array( 'ayin_--global--color-background' ),
				'ayin_--global--color-foreground' => array( 'ayin_--global--color-background' ),
				'ayin_--global--color-background' => array( 'ayin_--global--color-foreground' ),
			),
		);

		wp_enqueue_script( $handle, $src, $deps );
		wp_script_add_data( $handle, 'data', sprintf( 'var ayinValidateWCAGColorContrastExports = %s;', wp_json_encode( $exports ) ) );

		// Custom color contrast validation text
		wp_localize_script( $handle, 'ayinValidateContrastText', esc_html__( 'This color combination may be hard for people to read. Try using a brighter background color and/or a darker foreground color.', 'ayin' ) );
	}
